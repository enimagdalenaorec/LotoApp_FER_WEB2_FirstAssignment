...
1) Add a round:
curl -X POST https://localhost:7271/new-round -H "Authorization: Bearer example_token"

2) Close a round:
curl -X POST https://localhost:7271/close -H "Authorization: Bearer example_token"

3) Gives drawn numbers (List of int values):
curl -X POST https://localhost:7271/store-results -H "Authorization: Bearer example_token" -H "Content-Type: application/json" -d "{\"numbers\":[5,12,27,34,41,40]}"
